...
char *getMessage(int fd) {
  ...
  *tmp = '\0';
  ...
  while ((end = getline(&tmp, &size, sstream)) > 0) {
    // If the line its read is a caridge return and a new line were at the end
    // of the header so break
    if (strcmp(tmp, "\r\n") == 0) {
      break;
    }
...
// Extracts the filename needed from a GET request and adds public_html to the
// front of it
...
httpRequest parseRequest(char *msg) {
  ...
  // If they asked for a specific page and it exists because we opened it
  // sucessfully return it
  else if (exists != NULL) {
...
int printFile(int fd, char *filename) {
  ...
  /* Open the file filename and echo the contents from it to the file descriptor
   * fd */
  ...
  // Variable for getline to write the size of the line its currently printing
  // to
  size_t size = 1;
...
int main(int argc, char *argv[]) {
  ...
  // set all bytes in socket address structure to zero, and fill in the relevant
  // data members
  ...
  while (1) {
    ...
    if (pid == 0) {
      // Have the child loop infinetly dealing with a connection then getting
